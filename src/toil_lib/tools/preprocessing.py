...
def run_gatk_preprocessing(job, bam, bai, ref, ref_dict, fai, phase, mills, dbsnp, unsafe=False):
    ...
    mdups_disk = PrR(lambda bam_id, bai_id: 2 * (bam_id.size + bai_id.size), bam, bai)
    ...
    realigner_target_disk = PrR(lambda bam_id, bai_id, ref_size:
                                2 * bam_id.size + bai_id.size + ref_size,
                                mdups.rv(0),
                                mdups.rv(1),
                                indel_ref_size)
    ...
    indel_realign_disk = PrR(lambda bam_id, bai_id, intervals, ref_size:
                             2 * (bam_id.size + bai_id.size) + intervals.size + ref_size,
                             mdups.rv(0),
                             mdups.rv(1),
                             realigner_target.rv(),
                             indel_ref_size)
    ...
    base_recal_disk = PrR(lambda bam_id, bai_id, ref_size:
                          2 * bam_id.size + bai_id.size + ref_size,
                          indel_realign.rv(0),
                          indel_realign.rv(1),
                          bqsr_ref_size)
    ...
    recalibrate_reads_disk = PrR(lambda bam_id, bai_id, recal, ref_size:
                                 2 * (bam_id.size + bai_id.size) + recal.size + ref_size,
                                 indel_realign.rv(0),
                                 indel_realign.rv(1),
                                 base_recal.rv(),
                                 genome_ref_size)
